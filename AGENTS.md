...
- 로그인 후 랭킹 화면은 홈의 날짜·인사말과 같은 타이포그래피 구조로 기간을 `8월 3일 ~ 8월 9일` 형식으로 `랭킹` 제목 위에 표시하고, 스크롤 상단에 고정되는 `내 랭킹`, `전체 랭킹` 순서로 구성한다. 순위·운동한 날·횟수·인정 시간은 내 랭킹 행 한 곳에 모으고 별도 Hero 요약과 통계 타일로 반복하지 않는다. 내 랭킹과 전체 랭킹은 기존 Card·섹션 헤더·랭킹 행 표현을 공유하며 별도 포디움 같은 새로운 시각 문법을 추가하지 않는다.
