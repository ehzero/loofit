...
- 랭킹 로그인 유도 화면은 `이번 주의 꾸준함`, `내 순위 한눈에`, `매주 새로운 동기`를 중심으로 로그인 가치를 적극적으로 안내한다. 영업 문구는 향후 Kakao 외 Apple·Google 등 로그인 제공자가 추가되어도 유지할 수 있도록 provider 중립적으로 작성하고, 로그인 버튼 문구는 `Apple로 로그인`, `카카오로 로그인` 형식으로 통일한다. 두 소셜 로그인 버튼은 44pt 높이·12pt 모서리를 공유하고 심볼과 레이블을 하나의 그룹으로 가운데 배치한다. Apple은 시스템이 관리하는 공식 버튼 타이포그래피와 심볼 크기를 사용하고, 카카오 버튼은 Apple의 시각적 크기에 맞춘 17pt·500 weight 시스템 글꼴, 16pt 공식 로그인 말풍선 심볼, 4pt 심볼 간격을 사용한다. 로그인 액션 전에 익명 주간 랭킹 자동 참여를 안내하며 현재 미수집 프로필 항목이나 향후 서버 동기화 범위를 제한하는 문구는 표시하지 않는다.
- 로그인 후 랭킹 화면은 홈의 날짜·인사말과 같은 타이포그래피 구조로 기간을 `8월 3일 ~ 8월 9일` 형식으로 `랭킹` 제목 위에 표시하고, 스크롤 상단에 고정되는 `내 랭킹`, `전체 랭킹` 순서로 구성한다. 기간은 서버 응답 전에 한국시간 현재 주를 계산해 인증 확인·랭킹 로딩·오류·로그인 유도 상태에서도 같은 위치와 높이로 즉시 표시하고 응답 후 레이아웃 시프트를 만들지 않는다. 순위·운동한 날·횟수·인정 시간은 내 랭킹 행 한 곳에 모으고 별도 Hero 요약과 통계 타일로 반복하지 않는다. 내 랭킹과 전체 랭킹은 기존 Card·섹션 헤더·랭킹 행 표현을 공유하며 별도 포디움 같은 새로운 시각 문법을 추가하지 않는다.
- 랭킹 최하단 집계 문구와 설정 최하단 저작권 문구는 `caption`·500 weight·`hint` 색상·가운데 정렬 스펙을 공유한다. 두 화면의 하단 문구는 한국어 단어 단위로 줄바꿈하며 설정의 브랜드 태그라인도 같은 줄바꿈·정렬 정책을 사용한다.
- 로그인한 회원은 한국시간 월요일 시작 주간 랭킹에 자동 참여한다. 하루 완료 운동 합계 5분 이상을 운동한 날로 인정하고 운동한 날 수를 우선하며, 같은 일수에서는 하루 최대 2시간의 운동 시간을 동점 기준으로 사용한다. 다른 회원에게는 매주 바뀌는 `루핏 XXXX` 표시명과 순위·운동한 날·횟수·인정 시간만 공개하고 본인 행은 `나`로 표시한다. 상세 계약은 `docs/ranking-contract.md`를 따른다.
...
- 서버 백업 기록에서 계산하는 주간 익명 랭킹 조회
- 설정에서 소셜 재인증 후 서버 계정·백업·랭킹·연결을 삭제하는 회원 탈퇴

아직 자동 다중 기기 동기화, 계정 전환, 서버 백업으로 로컬 전체 교체는 포함하지 않는다.
...
- 서버 영속 저장소는 DynamoDB 온디맨드를 사용한다. 사용자 원본 데이터는 `loofit-production-user-data`에 저장하고 35일 PITR을 유지하며, 재생성 가능한 랭킹은 `loofit-production-leaderboard`와 `byScore` GSI에 저장하고 TTL로 정리한다. 랭킹 테이블의 `byUser` GSI는 회원 탈퇴 시 사용자 귀속 item을 역조회하는 용도로만 사용한다.
...
- Apple 회원 탈퇴용 `/loofit/production/auth/apple` SecureString은 `teamId`, `keyId`, Sign in with Apple PKCS#8 `.p8` `privateKey` JSON으로 관리한다. 로그인 검증에는 이 키를 사용하지 않고 authorization code 교환과 revoke에만 사용하며 CDK·Git·로그에 값을 포함하지 않는다.
...
- 설정의 회원 탈퇴는 현재 가입 provider로 재인증한 뒤 루핏 계정을 삭제 요청한다. 요청이 서버에 접수된 경우에만 SecureStore 세션을 삭제하며, 서버 사용자·identity·모든 세션·운동 기록 백업·랭킹 데이터는 비동기 작업으로 영구 삭제하고 Kakao 연결 또는 Apple 승인을 해제한다. 로컬 SQLite 운동 기록과 루틴은 유지한다.
...
- 랭킹은 사용자 데이터 테이블 `KEYS_ONLY` Stream의 `BACKUP#WORKOUTS` revision 변경으로 집계한다. worker는 현재 사용자의 완료 기록을 한국시간 주간 기준으로 재계산하고 `ACTIVE` 사용자 조건과 랭킹 쓰기를 단일 transaction으로 실행한다. `sourceRevision` 조건으로 오래된 결과의 덮어쓰기를 막으며 score 없는 marker로 미집계 revision도 기억한다. `GET /v1/leaderboards/weekly`는 상위 50명과 내 순위를 반환하고 기존 item이 현재 backup revision보다 오래되면 현재 사용자만 먼저 재집계한다.
...
- Google Play 앱 콘텐츠는 운동 기능은 로그인 없이 접근 가능하지만 랭킹 탭은 카카오 로그인이 필요한 상태로 선언한다. Apple 로그인은 iOS 전용이므로 Android 심사 접근 수단에는 포함하지 않는다. 개인정보 처리방침은 `https://ehzero.github.io/loofit-legal/privacy/`, 고객지원은 `https://ehzero.github.io/loofit-legal/support/`, 외부 계정 삭제 요청은 `https://ehzero.github.io/loofit-legal/account-deletion/`을 사용하며 심사 접근 절차를 함께 검토한다.
