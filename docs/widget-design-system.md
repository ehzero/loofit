...
Small `루틴 진행`은 남는 세로 공간이 항목 사이에 과도하게 몰리지 않도록 표시 개수별 레시피를 사용한다. 1개는 중앙 정렬하고, 2개는 `lg` 고정 간격을 둔 그룹을 중앙 정렬하며, 3개는 기본 `xl` 제목으로 공간을 균등 배분한다. 4개는 모든 분할을 유지하기 위해 제목을 `lg`, 세로 content padding을 `md`로 낮춘 compact 밀도에서 공간을 균등 배분한다. 잠금화면 가로 목록의 최대 3개 정책과 크기는 변경하지 않는다.

`week`, `month`, `year` variant는 기간·정렬·요약 같은 제품 동작만 정의하고, 렌더링 수치는 `detailed`, `expanded`, `compact` 스타일에서 상속한다. 이번 달 위젯은 `detailed` 스타일을 사용하며 헤더에 연도 없이 `{월} · {횟수}회`를 표시하고 4~6행을 컨테이너 안에 맞추도록 셀 크기를 동적으로 계산한다. 6행에서는 7열 너비를 유지하면서 제목 아래 간격은 `sm`, 세로 셀 간격은 `xs`, 요일 행 높이는 `sm` 텍스트 line-height로 줄이고 날짜 셀 높이만 남은 세로 공간에 맞춘다. Medium 지난 4주 상세 위젯은 `expanded` 스타일을 사용하고 4개 달력 행을 남은 세로 공간에 균등 배치한다. 두 항목 모두 앱 내 미리보기와 실제 iOS·Android 위젯으로 제공한다.
...
이번 달 위젯의 `{월} · {횟수}회` 타이틀은 `textMedium` 색상에 `muted` 투명도를 합성한다. RN 미리보기, SwiftUI, Android Canvas는 원시 색상뿐 아니라 이 투명도까지 같은 계약 값으로 사용한다.
