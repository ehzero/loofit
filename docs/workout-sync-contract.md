...
현재 구현은 단방향 업로드와 사용자가 확인하는 병합 복원까지 담당한다. 랭킹은 업로드 batch가 증가시킨 backup revision을 DynamoDB Stream에서 감지해 별도 파생 집계하며 `ranking-contract.md`를 따른다. 자동 다중 기기 동기화, 계정 전환, 서버 백업으로 로컬 전체 교체는 후속 범위다.
