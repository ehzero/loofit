...
- 로그인 유도 화면의 영업 문구는 provider 중립적으로 작성하고 실제 로그인 버튼만 현재 제공자를 표시한다. 로그인 액션 전에는 회원이 매주 바뀌는 익명 표시명으로 주간 랭킹에 자동 참여한다는 사실을 안내하며 현재 미수집 프로필 항목이나 향후 운동 기록 동기화 범위를 제한하는 문구는 표시하지 않는다.
- 로그인 후 `GET /v1/leaderboards/weekly`로 실제 상위 50명과 내 순위를 조회한다. 가짜 사용자·순위를 실제 데이터처럼 렌더링하지 않으며 집계 대상 기록이 없거나 참가자가 없는 경우 전용 빈 상태를 표시한다. 점수·익명 표시·공개 필드는 `ranking-contract.md`를 따른다.
